...
def generate(images, shape):
    ...
    for img in images: 
        ...
        if not get_label(attack_image) == get_label(img):
            ...
            ssim_sum += ssim_img
            now_end = time.time()
            print("攻击结果: ", ssim_img, "耗时: ", now_end - now_start)
...
def change_pixel(image, time=1):
    ...
    for i in range(5):
        ...
        for j in range(y, row):
            new_image[x][j] = random.randint(0, 255)
        for j in range(x, col):
            new_image[j][y] = random.randint(0, 255)
...
if __name__ == '__main__':
    ...
    train_images, train_labels, test_images, test_labels = load_data()
     ...
     # 开始攻击
    print("----攻击开始----")
    images = generate(test_images[0:5], (1000, 28, 28, 1))
    print("----攻击结束----")
